...
export default function TrackerList() {
  ...
  const recent = trackers.slice(0, 4)

  return (
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
        <div>
          <h3 className="font-display font-semibold text-slate-900">Recent Trackers</h3>
          <p className="text-slate-500 text-sm mt-0.5">Your linked devices and their status</p>
        </div>
        <button
      ...
      <div className="divide-y divide-slate-100">
        {loading ? (
          ...
          <div className="py-10 text-center text-slate-400 text-sm">No trackers yet</div>
        ) : (
          recent.map((tracker, i) => (
            <motion.div
              key={tracker.id}
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">{tracker.last_seen}</p>
                </div>
              </div>
                  ...
                  <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      className={`h-full rounded-full ${
                        tracker.battery > 50 ? 'bg-emerald-500' :
                        tracker.battery > 20 ? 'bg-amber-500' : 'bg-red-500'
                      }`}
                    />
                  </div>
                </div>
